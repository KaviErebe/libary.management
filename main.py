...
if b==False:
    cur.execute("create table Library(BK_NAME varchar(30),BK_ID int PRIMARY KEY NOT NULL,AUTHOR_NAME varchar(30),BK_STATUS varchar(30), CARD_ID varchar(30))")
...
def display_records():
	...
	curr = cur.execute('SELECT * FROM Library')
	data = cur.fetchall()
...
def add_record():
	...
	if surety:
		try:
			cur.execute('INSERT INTO Library (BK_NAME, BK_ID, AUTHOR_NAME, BK_STATUS, CARD_ID) VALUES (%s, %s, %s, %s, %s)',(bk_name.get(), bk_id.get(), author_name.get(), bk_status.get(), card_id.get()))
			con.commit()

			clear_and_display()

			mb.showinfo('Record added', 'The new record was successfully added to your database')
		except msql.IntegrityError:
...
def update_record():
	def update():
		...
		cur.execute('UPDATE Library SET BK_NAME=%s, BK_STATUS=%s, AUTHOR_NAME=%s, CARD_ID=%s WHERE BK_ID=%s',
...
def remove_record():
	...
	cur.execute('DELETE FROM Library WHERE BK_ID=%s', (selection[1], ))
	con.commit()
...
def change_availability():
	...
	if BK_status == 'Issued':
		...
		if surety:
			cur.execute('UPDATE Library SET bk_status=%s, card_id=%s WHERE bk_id=%s', ('Available', 'N/A', BK_id))
			con.commit()
		else: mb.showinfo(
			'Cannot be returned', 'The book status cannot be set to Available unless it has been returned')
	else:
		cur.execute('UPDATE Library SET bk_status=%s, card_id=%s where bk_id=%s', ('Issued', issuer_card(), BK_id))
		con.commit()
